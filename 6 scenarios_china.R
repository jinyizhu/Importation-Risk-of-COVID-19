...
## simulated mainland china imported cases
simulate_input <- read.xlsx('sim_chn_20200525.xlsx')
...
npop = 1400000000-59270000  # population
R_0=29745+2715-20912-2615  # removed cases
...
# Rt
Rt_df <- read.csv('estimate_rt_chn.csv',as.is = T)
...
for (i in c(1:length(input_all))){
  ...
  for (t in c(2:tmax)){
    ...
    results[t,"all_new_case"] <- results[t,"new_case"]+results[t,"under_case_all"]                   ## under report cases
    results[t,"all_new_case_import"] <- results[t,"new_input"]+results[t,"under_case_import"]        ## under report cases
...
for(k in c(k1,k2,k3)){
  ...
  for (t in c(2:t1)){
    ...
    input = data.frame(new_input=inputcase*(1-prop_a)*(1-prop_e)+inputcase*prop_a,
                       nfree_I=0,  ##not free
                       free_Is=inputcase*(1-prop_a)*(1-prop_e),
  ...
  for (t in c((t1+1):t2)){
    ...
    solved_model = as.data.frame(lsoda(inits, seq(1,t2-t1+1), SEIR_func, vparameters))
